...
class Blockchain:
    ...
    def create_block(self, proof, previous_hash, value=None):
        block = {
    ...
    def is_chain_valid1(self, chain):
        previous_block = chain[0]
        block_index = 1
        ...
        while block_index < len(chain):
            ...
            previous_proof = previous_block['proof']
            proof = block['proof']
            hash_op = self.hash_op(proof, previous_proof)
            if self.is_hash_valid(hash_op) is True:
                return False
            previous_block = block
            block_index += 1
